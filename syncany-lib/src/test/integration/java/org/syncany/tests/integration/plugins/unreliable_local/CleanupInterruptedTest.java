...
public class CleanupInterruptedTest {
	...
	public void testUnreliableCleanup_Test1_oldVersionRemoval() throws Exception {
		...
		assertEquals(2, transferManagerA.list(PathAwareRemoteFileType.Multichunk, RemoteFileFactories::createMultichunkFile).size());
		assertEquals(1, new File(testConnection.getPath(), "multichunks").list().length);
		assertEquals(2, transferManagerA.list(PathAwareRemoteFileType.Database, RemoteFileFactories::createDatabaseFile).size());
		assertEquals(0, new File(testConnection.getPath(), "databases").list().length);
		assertEquals(1, transferManagerA.list(PathAwareRemoteFileType.Transaction, RemoteFileFactories::createTransactionFile).size());
		assertEquals(1, new File(testConnection.getPath(), "transactions").list().length);
		assertEquals(1, transferManagerA.list(PathAwareRemoteFileType.Action, RemoteFileFactories::createActionFile).size());
		assertEquals(1, new File(testConnection.getPath(), "actions").list().length);

		clientA.cleanup(cleanupOptions);
		assertEquals(1, transferManagerA.list(PathAwareRemoteFileType.Multichunk, RemoteFileFactories::createMultichunkFile).size());
		assertEquals(1, new File(testConnection.getPath(), "multichunks").list().length);
		assertEquals(1, transferManagerA.list(PathAwareRemoteFileType.Database, RemoteFileFactories::createDatabaseFile).size());
		assertEquals(1, new File(testConnection.getPath(), "databases").list(new FilenameFilter() {
			public boolean accept(File dir, String name) {
				return name.startsWith("database-");
			}
		}).length);
		assertEquals(0, transferManagerA.list(PathAwareRemoteFileType.Transaction, RemoteFileFactories::createTransactionFile).size());
		assertEquals(0, new File(testConnection.getPath(), "transactions").list().length);
		assertEquals(0, transferManagerA.list(PathAwareRemoteFileType.Action, RemoteFileFactories::createActionFile).size());
	...
	public void testUnreliableCleanup_Test2_databaseFileMerge() throws Exception {
		...
		for (AbstractRemoteFile remoteFile : transferManagerA.list(PathAwareRemoteFileType.Database, RemoteFileFactories::createDatabaseFile)) {
	...
	public void testUnreliableCleanup_Test3_unreferencedTempFiles() throws Exception {
		...
		// One deletion failed
		assertEquals(1, transferManagerA.list(PathAwareRemoteFileType.Temp, RemoteFileFactories::createTempFile).size());
		...
		assertEquals(0, transferManagerA.list(PathAwareRemoteFileType.Temp, RemoteFileFactories::createTempFile).size());
