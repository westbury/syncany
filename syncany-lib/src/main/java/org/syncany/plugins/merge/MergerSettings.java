 ...
 * 
 * <p>Options for a plugin specific {@link MergerSettings} can be defined using the
 * {@link Element} annotation. Furthermore some Syncany-specific annotations are available.
 *
 * @author Philipp C. Heckel <philipp.heckel@gmail.com>
 * @author Christian Roth <christian.roth@port17.de>
 */
public abstract class MergerSettings {
	private static final Logger logger = Logger.getLogger(MergerSettings.class.getName());
	 ...
	 *
	 * @param key The field name as it is used in the {@link MergerSettings}
	 * @return The value converted to a string using {@link Class#toString()}
	 * @throws MergingException Thrown if the field either does not exist or isn't accessible
	 */
	public final String getField(String key) throws MergingException {
		try {
			...
			throw new MergingException("Unable to getField named " + key + ": " + e.getMessage());
		}
	}
	 ...
	 *              {@link MergerSettings} are converted.
	 * @throws MergingException Thrown if the field either does not exist or isn't accessible or
	 *         conversion failed due to invalid field types.
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public final void setField(String key, Object value) throws MergingException {
		try {
			...
			for (Field field : elementFields) {
				...
				if (key.equalsIgnoreCase(fieldName)) {
					...
					else if (MergerSettings.class.isAssignableFrom(value.getClass())) {
			...
			throw new MergingException("Unable to parse value because its format is invalid: " + e.getMessage(), e);
		}
	}

	/**
	 * Check if a {@link MergerSettings} instance is valid i.e. all required fields are present.
	 * {@link MergerSettings} specific validators can be deposited by annotating a method with {@link Validate}.
	 *
	 * @return True if the {@link MergerSettings} instance is valid.
	 */
	public final boolean isValid() {
		...
		try {
			...
			if (e.getCause() instanceof MergingException) { // Dirty hack
	...
	/**
	 * Get the reason why the validation with {@link MergerSettings#isValid()} failed.
	 *
	 ...
	 *
	 * @throws MergingException Thrown if the validation failed due to missing field values.
	 */
	@Validate
	public final void validateRequiredFields() throws MergingException {
		...
		try {
			...
			for (Field field : elementFields) {
				...
				if (field.getAnnotation(Element.class).required() && field.get(this) == null) {
					...
					throw new MergingException("Missing mandatory field " + this.getClass().getSimpleName() + "#" + field.getName());
				}
			}
		}
	...
	private String findPluginId() {
		MergerPlugin mergerPlugin = getMergerPlugin();

		try {
			if (mergerPlugin != null) {
				return mergerPlugin.getId();
			}
	...
	// Can create a new instance each time if it wants....
	public abstract MergerPlugin getMergerPlugin();
